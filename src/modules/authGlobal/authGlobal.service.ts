...
export class AuthGlobalService {
  ...
  async signup(user: CreateUserDto): Promise<Omit<User, 'password' | 'role'>> {
    // Si tiene email, Compruebo que el email de usuario no este ya creado, sino devuelve un error
    if (user.email) {    
        const userDB = await this.usersRepository.getUserByEmailRepository(user.email);
        if (userDB)
          throw new BadRequestException(
            `El usuario con el email ${userDB.email} ya existe`,
          );
        }
    ...
    const { passwordConfirm, rol, ...createUser } = user;
    // busco el role asignado, si no existe devuelvo error
    const userRole: UserRole = await this.usersRepository.getRolesUsersByRoleRepository(rol);
    if (!userRole) throw new NotFoundException('El rol Asignado no Existe');

    // creo el usuario en la DB pisando el dato del password con la clave hasheada y agregando la relacion role
