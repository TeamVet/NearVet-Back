...
export class VeterinarianService {
  ...
  async getVeterinarianById(id: string): Promise<Veterinarian> {
    ...
    if (!vet) {throw new NotFoundException("El Veterinario buscado no existe")}
    return vet;
  }
  ...
  async create(createVeterinarian: Partial<Veterinarian>): Promise<Veterinarian> {
    ...
    if (!vetCreated) {throw new InternalServerErrorException(`La creacione del veterinario no pudo concretarse`)}
    return vetCreated
  } 
